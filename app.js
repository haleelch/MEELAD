...
(function () {
  ...
  function drawResultPoster(event, winners, templateOverride) {
    ...
    const paintWinners = () => {
      ...
      const gap = canvas.height * 0.14;
      winners.forEach((w, i) => {
        const blockY = startY + i * gap;

        ctx.textAlign = "center";
        ctx.shadowBlur = 0;
        ctx.fillStyle = "rgba(0,0,0,.55)";
        ctx.beginPath(); ctx.arc(cx, blockY, 30, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#EC4899"; ctx.font = "700 30px Poppins, sans-serif";
        ctx.fillText(String(i + 1), cx, blockY + 11);

        ctx.shadowColor = "rgba(0,0,0,.6)"; ctx.shadowBlur = 8;
        ctx.fillStyle = "#fff"; ctx.font = "700 34px Poppins, sans-serif";
        ctx.fillText(w.student.name.toUpperCase(), cx, blockY + 70);

        ctx.shadowBlur = 0;
        ctx.font = "400 21px Poppins, sans-serif"; ctx.fillStyle = "#E5E7EB";
        ctx.fillText(w.team ? w.team.name : "", cx, blockY + 102);
      });
    };
