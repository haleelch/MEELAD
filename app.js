...
(function () {
  ...
  const CATEGORY_START = { "Sub Junior": 100, Junior: 200, Senior: 300, "Super Senior": 400 };
  let state = {
    hero: seed.hero,
  ...
  function ensureStateDefaults() {
    ...
    state.categories.forEach((c) => {
      const start = CATEGORY_START[c] || 100;
  ...
  function nextChestNo(category) {
    const start = CATEGORY_START[category] || 100;
  ...
  function renderHero() {
    ...
    document.getElementById("sideBrandTitle").textContent = state.hero.title;
    ...
    document.getElementById("heroTitle").style.display = showText ? "" : "none";
  ...
  function openAdminSidebar() { adminSidebar.classList.add("open"); adminSidebarOverlay.classList.remove("hidden"); }
  ...
  document.getElementById("btnCloseAdminSidebar").addEventListener("click", closeAdminSidebar);
  ...
  function renderPosterTab() {
    ...
    adminContent.innerHTML = `
      <div class="card">
        ...
        <div class="field-label" style="margin-top:.4rem">Fest Title (used as a fallback banner, and in WhatsApp share text)</div>
      ...
      <div class="card">
        <div class="card-title">Other Text</div>
        <div class="field-label">Badge Label (small text above the title on the home banner)</div>
        <input id="fBadge" class="input" style="margin-bottom:.5rem" value="${escapeAttr(h.badge || "ANNUAL FESTIVAL")}" />
        <div class="field-label">Subtitle (paragraph shown under the title on the home banner)</div>
        ...
        <label style="display:flex;align-items:center;gap:.5rem;margin-bottom:.75rem;cursor:pointer">
          ...
          <span class="field-label" style="margin:0">Show badge, title &amp; subtitle text on the home banner (turn off if your poster photo already has this text on it)</span>
        </label>
        <button class="btn btn-primary" id="btnSavePoster" style="width:auto;padding:.6rem 1rem">\u2713 Save</button>
      </div>
  ...
  function renderChestNoTab() {
    adminContent.innerHTML = `
      <div class="card">
        ...
        <div class="field-label" style="margin-bottom:.6rem">Numbers are auto-increment only and are <b>never reused</b>, even after a student is deleted \u2014 the next student always gets the next fresh number.</div>
        <div class="marks-table-wrap">
          <table class="marks-table">
            <thead><tr><th>Category</th><th>Active</th><th>Next No.</th></tr></thead>
            <tbody>
              ${state.categories.map((c) => {
                const active = state.students.filter((s) => s.category === c).length;
                const next = state.chestCounters[c] || CATEGORY_START[c] || 100;
                return `<tr><td style="text-align:left">${escapeHtml(c)}</td><td>${active}</td><td style="font-family:'JetBrains Mono',monospace">${next}</td></tr>`;
              }).join("")}
            </tbody>
          </table>
        </div>
