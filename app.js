...
(function () {
  ...
  function drawResultPoster(event, winners, templateOverride) {
    ...
    const paint = () => {
      ...
      // header — just "Result", lighter weight (not the heavy display font used before)
      ctx.shadowColor = "rgba(0,0,0,.55)"; ctx.shadowBlur = 8;
      ctx.fillStyle = "#fff";
      ctx.font = "600 60px Poppins, sans-serif";
      ctx.fillText("Result", cx, 170);

      // winners — icon on top, name centered directly below it, team below that
      const startY = 340;
      const blockGap = 230;
      winners.forEach((w, i) => {
        const blockY = startY + i * blockGap;

        ctx.shadowBlur = 0;
        ctx.fillStyle = "rgba(0,0,0,.6)";
        ctx.beginPath(); ctx.arc(cx, blockY, 34, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#EC4899"; ctx.font = "700 34px Poppins, sans-serif";
        ctx.fillText(String(i + 1), cx, blockY + 12);

        ctx.shadowColor = "rgba(0,0,0,.55)"; ctx.shadowBlur = 8;
        ctx.fillStyle = "#fff"; ctx.font = "700 36px Poppins, sans-serif";
        ctx.fillText(w.student.name.toUpperCase(), cx, blockY + 78);

        ctx.shadowBlur = 0;
        ctx.font = "400 23px Poppins, sans-serif"; ctx.fillStyle = "#D1D5DB";
        ctx.fillText(w.team ? w.team.name : "", cx, blockY + 112);
      });
    };
