...
(function () {
  ...
  function renderHero() {
    ...
    if (state.hero.photoUrl) {
      heroSection.style.backgroundImage = `linear-gradient(180deg, rgba(11,61,46,.55), rgba(7,15,11,.92)), url('${state.hero.photoUrl}')`;
  ...
  function openPosterModal({ student, rank, event, team }) {
    ...
    modalBody.innerHTML = `
      ...
      <div style="padding:.85rem 1rem 0;background:var(--surface)">
        <div class="field-label" style="margin-bottom:.4rem">Choose a poster design</div>
        <div class="template-row" id="pickTemplateRow">
          ${templates.map((t) => `<div class="template-swatch ${t.id === selectedTemplate ? "selected" : ""}" data-tpl="${t.id}" style="background:${t.bg};color:${t.fg};text-shadow:0 1px 3px rgba(0,0,0,.6);min-height:2.6rem">${t.label}</div>`).join("")}
        </div>
      </div>` : ""}
  ...
  function renderPosterTab() {
    ...
    adminContent.innerHTML = `
      <div class="card">
        <div class="card-title">Home Page Photo</div>
        <div class="field-label">This is the first image parents see when they open the site. Each madrasa can set their own.</div>
      ...
      <div class="card">
        <div class="card-title">Home Page Poster Text</div>
