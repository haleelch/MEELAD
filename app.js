...
(function () {
  ...
  function renderHero() {
    ...
    if (footerEl) footerEl.textContent = `${state.hero.title} \u00a9 All rights reserved`;
  ...
  function drawResultPoster(event, winners, templateOverride) {
    ...
    // No template uploaded yet — plain background so the feature still works.
              ...
              <tbody>
                ${rows.map((student) => {
                  ...
                  <tr>
                    ...
                    ${event.assignedJudges.map((j) => `<td><input type="number" class="me-mark-input" data-student="${student.id}" data-judge="${escapeAttr(j)}" value="${marksSoFar[j] ?? ""}" /></td>`).join("")}
        ...
        <div class="checkbox-list" id="checkList">
          ${eligible.length === 0 ? `<div class="muted" style="font-size:.72rem;font-style:italic">No programmes match this category/gender.</div>` :
            eligible.map((ev) => {
              ...
              const disabled = !checked && ((ev.type === "Individual" && selIndiv >= state.limits.maxIndividual) || (ev.type === "Group" && (selGroup >= state.limits.maxGroup || groupTeamFull(ev.id, studentForm.team))));
        ...
        <div class="checkbox-list" id="pCheckList">
          ${eligible.length === 0 ? `<div class="muted" style="font-size:.72rem;font-style:italic">No programmes match this category/gender.</div>` :
            eligible.map((ev) => {
              ...
              const disabled = !checked && ((ev.type === "Individual" && selIndiv >= state.limits.maxIndividual) || (ev.type === "Group" && (selGroup >= state.limits.maxGroup || groupTeamFull(ev.id, student.team, student.id))));
  ...
  function renderScheduleTab() {
    const refreshEventOptions = () => {
      const cat = document.getElementById("schCategory").value;
      const evs = state.events.filter((e) => e.category === cat);
      document.getElementById("schEvent").innerHTML = evs.length
        ? evs.map((e) => `<option value="${e.id}">${escapeHtml(e.name)}</option>`).join("")
      ...
      wrap.innerHTML = Object.keys(groups).map((stage) => `
        <div class="card" style="padding:.7rem .85rem;margin-bottom:.6rem">
          ...
          ${groups[stage].map((s) => {
            ...
            return `<div style="display:flex;justify-content:space-between;align-items:center;padding:.4rem 0;border-top:1px solid var(--border)">
              <div>
                <div style="font-size:.85rem;font-weight:600">${ev ? escapeHtml(ev.name) : "(programme deleted)"}</div>
                <div style="font-size:.72rem;color:var(--muted)">${ev ? escapeHtml(ev.category) : ""} \u00b7 ${formatTimeRange(s.start, s.end)}</div>
              </div>
              <button class="icon-btn" data-del-sched="${s.id}" style="width:2rem;height:2rem;color:#EF4444">&#128465;</button>
            </div>`;
          }).join("")}
        </div>`).join("");
      wrap.querySelectorAll("[data-del-sched]").forEach((b) => b.addEventListener("click", () => {
    ...
    refreshEventOptions();
    document.getElementById("schCategory").addEventListener("change", refreshEventOptions);
    renderList();
  ...
  function openPrintSheet(kind, eventId, skipHistoryLog) {
    ...
    const participants = event.type === "Group" ? groupAwareParticipants(eventId) : state.students.filter((s) => s.events.includes(eventId));
    const now = new Date();
