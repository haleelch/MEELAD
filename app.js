...
(function () {
  ...
  // Result poster: category + programme name + top-3 winners on a light
  // (#F2F2F2) background with a soft white radial highlight top-right.
  // Programme title, winner name & team are drawn in white per request
  // (the reference HTML had them in black); category and rank-badge number
  // stay as given. Everything is drawn by code \u2014 no uploaded background.
  function drawResultPoster(event, winners) {
    ...
    const glow = ctx.createRadialGradient(CW + 78, -84, 0, CW + 78, -84, 1662);
    glow.addColorStop(0, "rgba(255,255,255,0.8)");
    glow.addColorStop(0.7, "rgba(242,242,242,0)");
    ctx.fillStyle = glow;
    ctx.beginPath(); ctx.arc(CW + 78, -84, 1662, 0, Math.PI * 2); ctx.fill();

    const PAD_L = 89, PAD_TOP = 249;
    ctx.textAlign = "left";
    ctx.fillStyle = "#F4A22A"; ctx.font = "600 72px Poppins, sans-serif";
    ctx.fillText(event.category || "", PAD_L, PAD_TOP + 72 * 0.85);

    ctx.fillStyle = "#fff"; ctx.font = "700 100px Poppins, sans-serif";
    ctx.fillText(event.name || "", PAD_L, PAD_TOP + 72 + 11 + 100 * 0.85);

    const BOX = 161, RADIUS = 33, ROW_GAP = 66, DETAILS_LEFT = PAD_L + BOX + 50;
    const ROW_H = BOX + ROW_GAP;
    const WINNERS_TOP = 567;
    ...
    winners.slice(0, 3).forEach((w, i) => {
      ...
      const numX = PAD_L + BOX / 2 - (numW + 30) / 2;
      ctx.fillText(String(i + 1), numX, centerY + 4);
      ctx.font = "600 39px Poppins, sans-serif";
      ctx.fillText(ordinalSuffix(i + 1), numX + numW + 2, centerY - 20);
      ctx.textBaseline = "alphabetic";

      ctx.fillStyle = "#fff"; ctx.font = "700 61px Poppins, sans-serif";
      ctx.fillText(w.student.name || "", DETAILS_LEFT, centerY - 5);

      ctx.fillStyle = "#fff"; ctx.font = "italic 500 44px Poppins, sans-serif";
      ctx.fillText(w.team ? w.team.name : "", DETAILS_LEFT, centerY + 56);
    });

    ctx.textAlign = "center";
    ctx.fillStyle = "#E0E0E0"; ctx.font = "400 30px Poppins, sans-serif";
    ctx.fillText("E V E N T   C R E W", CW / 2, CH - 83);
    ctx.textAlign = "left";
