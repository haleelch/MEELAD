...
(function () {
  ...
  const DEFAULT_CATEGORY_START = { "Sub Junior": 100, Junior: 200, Senior: 300, "Super Senior": 400 };
  let state = {
    hero: seed.hero,
    ...
    chestCounters: seed.chestCounters || {},
    deletedChestNumbers: seed.deletedChestNumbers || [],
  };
  ...
  function ensureStateDefaults() {
    ...
    state.chestCounters = state.chestCounters || {};
    state.deletedChestNumbers = state.deletedChestNumbers || [];
    ...
    // Every category needs a start number. Known legacy categories fall back to
    // their historical defaults; any brand-new category with no start number yet
    // gets the next free hundred above every other category's start.
    state.categories.forEach((c) => {
      if (state.categoryStartNumbers[c] == null) {
        if (DEFAULT_CATEGORY_START[c] != null) {
          state.categoryStartNumbers[c] = DEFAULT_CATEGORY_START[c];
        } else {
          const highestStart = Object.values(state.categoryStartNumbers).reduce((m, v) => Math.max(m, v), 0);
          state.categoryStartNumbers[c] = Math.floor(highestStart / 100) * 100 + 100;
        }
      }
    });
    // Chest-no counters must never fall behind any number already in use (current
    // students) or already retired (deleted log) — this is what guarantees numbers
    // are auto-incrementing only and are never repeated, even across deletions.
    state.categories.forEach((c) => {
      const start = state.categoryStartNumbers[c] || 100;
      const highestUsed = state.students.filter((s) => s.category === c)
        .reduce((max, s) => Math.max(max, parseInt(s.chestNo, 10) || 0), start - 1);
      const highestDeleted = state.deletedChestNumbers.filter((d) => d.category === c)
        .reduce((max, d) => Math.max(max, parseInt(d.chestNo, 10) || 0), start - 1);
      const floor = Math.max(highestUsed, highestDeleted) + 1;
      if (!state.chestCounters[c] || state.chestCounters[c] < floor) state.chestCounters[c] = floor;
    });
  }
  ensureStateDefaults();
  ...
  // Chest numbers are auto-increment only and NEVER reused, even after a student
  // is deleted. state.chestCounters[category] always holds the next number to
  // hand out; deletions are logged (not rolled back) in state.deletedChestNumbers.
  function nextChestNo(category) {
    const start = state.categoryStartNumbers[category] || 100;
    if (!state.chestCounters[category] || state.chestCounters[category] < start) state.chestCounters[category] = start;
    const chestNo = String(state.chestCounters[category]);
    state.chestCounters[category] += 1;
    return chestNo;
  }
  function logDeletedChestNo(student) {
    state.deletedChestNumbers.unshift({
      chestNo: student.chestNo, name: student.name, category: student.category, deletedAt: new Date().toISOString(),
    });
    if (state.deletedChestNumbers.length > 300) state.deletedChestNumbers.length = 300;
  }
  ...
  document.querySelectorAll(".side-link[data-nav]").forEach((a) => a.addEventListener("click", popScreen));
  ...
  /* ---------------- hero ---------------- */
  function renderHero() {
    // Page Header (top bar) — independent of the poster text below, always shown
    ...
    const showText = state.hero.showText !== false;
    document.getElementById("heroBadge").style.display = showText ? "" : "none";
    document.getElementById("posterHeading").style.display = showText ? "" : "none";
    document.getElementById("heroSubText").style.display = showText ? "" : "none";
  ...
  // Builds a hidden print-only sheet (8 cards per A4 page) for every student
  // in a category, then triggers the browser's print dialog.
  function printCategoryCards(category) {
    ...
    Promise.all(students.map((s) => drawIdCard(s))).then((urls) => {
      const old = document.querySelector(".print-cards-sheet");
      if (old) old.remove();
      const sheet = document.createElement("div");
      sheet.className = "print-cards-sheet";
      sheet.innerHTML = urls.map((url) => `<img class="print-card-img" src="${url}" />`).join("");
      ...
      setTimeout(() => { window.print(); setTimeout(cleanup, 1000); }, 200);
    }).catch(() => showToast("Could not prepare cards for printing"));
  }
  ...
  function openResultPosterModal(eventId) {
    ...
    document.getElementById("btnShareResult").addEventListener("click", () => {
      ...
      const text = encodeURIComponent(`Alhamdulillah! Results for ${event.name} (${event.category}) at ${state.hero.title} \u2014 ${summary}`);
      window.open(`https://wa.me/?text=${text}`, "_blank");
    });
  ...
  function openCardModal(student) {
    ...
    document.getElementById("btnShareCard").addEventListener("click", () => {
      const text = encodeURIComponent(`${student.name} \u2014 Chest No ${student.chestNo}, ${student.category} at ${state.hero.title}, representing ${team ? team.name : ""}!`);
      window.open(`https://wa.me/?text=${text}`, "_blank");
    });
  ...
  document.getElementById("btnAdminGuestView").addEventListener("click", popScreen);
  ...
  function renderPosterTab() {
    ...
    adminContent.innerHTML = `
      ...
      <div class="card">
        ...
        <div class="field-label">Upload your own designed poster (from Canva, Photoshop, etc.) \u2014 it replaces the whole banner at the top of the home page, exactly as you made it. Each madrasa can upload their own. No poster uploaded yet? A simple text banner is shown instead.</div>
        <div class="field-label" style="margin-top:.4rem">Poster Heading (also used as a fallback banner, and in WhatsApp share text)</div>
        <input id="fTitle" class="input" style="margin:.4rem 0 .75rem" value="${escapeAttr(h.title)}" />
  ...
  function renderCardsTab() {
    ...
    adminContent.innerHTML = `
      ...
      <div class="card">
        <div class="card-title">Bulk Print by Category (A4)</div>
        <div class="field-label" style="margin-bottom:.5rem">Pick a category \u2014 every registered student in it gets their ID card printed on A4 sheets, 8 cards per page.</div>
        ...
        <button class="btn btn-primary" id="btnPrintCategoryCards" style="width:auto;padding:.5rem .9rem">\u{1F5A8} Print All (A4)</button>
      </div>
  ...
  function renderStudentsList() {
    ...
    document.querySelectorAll("#studentsListWrap .trash-btn").forEach((b) => b.addEventListener("click", (e) => {
      ...
      if (student) logDeletedChestNo(student);
  ...
  function renderChestNoTab() {
    adminContent.innerHTML = `
      <div class="card">
        <div class="card-title">Auto Chest Numbers</div>
        <div class="field-label" style="margin-bottom:.6rem">Numbers are auto-increment only and are <b>never reused</b>, even after a student is deleted \u2014 the next student always gets the next fresh number. Change a Start Number below and Save to apply it (it can't be set lower than numbers already in use).</div>
        <div class="marks-table-wrap">
          <table class="marks-table">
            <thead><tr><th>Category Name</th><th>Active</th><th>Start Number</th><th></th></tr></thead>
            <tbody>
      ...
      <div class="card">
        ...
        <div class="field-label" style="margin-bottom:.5rem">A new category automatically starts at the next free hundred above every existing category (e.g. if the last one starts at 400, the new one starts at 500).</div>
      ...
      <div class="card">
        <div class="card-title">Search by Chest No. or Name</div>
        <div style="display:flex;gap:.5rem;margin-bottom:.5rem">
          <input id="chestSearch" class="input" placeholder="e.g. 103 or student name" style="flex:1" />
          <button class="btn btn-primary" id="btnChestSearch" style="width:auto;padding:.5rem .9rem">\u{1F50D}</button>
        </div>
        <div id="chestSearchResults"></div>
      </div>
      ...
      <div class="card">
        <div class="card-title">Deleted / Retired Numbers Log</div>
        <div class="field-label" style="margin-bottom:.5rem">These chest numbers were assigned once and then freed up by deletion \u2014 they are kept here for record only and will not be given to anyone else.</div>
        ${state.deletedChestNumbers.length === 0 ? `<div class="muted" style="font-size:.72rem;font-style:italic">No deletions yet.</div>` : `
        <div class="marks-table-wrap">
          <table class="marks-table">
            <thead><tr><th>Chest No</th><th>Name</th><th>Category</th><th>Deleted</th></tr></thead>
            <tbody>
              ${state.deletedChestNumbers.map((d) => `<tr><td style="font-family:'JetBrains Mono',monospace">${d.chestNo}</td><td style="text-align:left">${escapeHtml(d.name)}</td><td>${escapeHtml(d.category)}</td><td style="font-size:.65rem">${new Date(d.deletedAt).toLocaleDateString("en-GB")}</td></tr>`).join("")}
            </tbody>
          </table>
        </div>`}
      </div>`;
    ...
    document.querySelectorAll(".btn-save-start").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const highestUsed = state.students.filter((s) => s.category === cat)
          .reduce((max, s) => Math.max(max, parseInt(s.chestNo, 10) || 0), 0);
        const highestDeleted = state.deletedChestNumbers.filter((d) => d.category === cat)
          .reduce((max, d) => Math.max(max, parseInt(d.chestNo, 10) || 0), 0);
        const minAllowed = Math.max(highestUsed, highestDeleted) + 1;
        if (val < minAllowed && (highestUsed || highestDeleted)) {
          return showToast(`Can't go below ${minAllowed} \u2014 numbers up to ${Math.max(highestUsed, highestDeleted)} are already in use for this category`);
        }
        state.categoryStartNumbers[cat] = val;
        if (!state.chestCounters[cat] || state.chestCounters[cat] < val) state.chestCounters[cat] = val;
        persist();
    ...
    document.getElementById("btnAddCategory").addEventListener("click", () => {
      ...
      state.chestCounters[name] = nextStart;
      persist();
    ...
    function runSearch() {
      const q = document.getElementById("chestSearch").value.trim().toLowerCase();
      const wrap = document.getElementById("chestSearchResults");
      if (!q) { wrap.innerHTML = ""; return; }
      const matches = state.students.filter((s) => s.chestNo.toLowerCase().includes(q) || s.name.toLowerCase().includes(q));
      if (!matches.length) { wrap.innerHTML = `<div class="muted" style="font-size:.72rem;font-style:italic">No student found.</div>`; return; }
      wrap.innerHTML = matches.map((s) => {
        const team = state.teams.find((t) => t.id === s.team);
        return `<div class="card student-card" data-open="${s.id}" style="cursor:pointer;margin-bottom:.5rem">
          <div style="font-size:.85rem;font-weight:500">${escapeHtml(s.name)} <span style="font-family:'JetBrains Mono',monospace;font-size:.72rem;color:var(--gold)">${s.chestNo}</span></div>
          <div class="muted" style="font-size:.7rem">${s.category} \u00b7 ${s.gender} \u00b7 ${team ? escapeHtml(team.name) : ""}</div>
        </div>`;
      }).join("");
      wrap.querySelectorAll(".student-card").forEach((card) => card.addEventListener("click", () => {
        studentsView = { mode: "profile", id: card.dataset.open };
        setActiveAdminTab("students");
      }));
    }
    document.getElementById("btnChestSearch").addEventListener("click", runSearch);
    document.getElementById("chestSearch").addEventListener("keydown", (e) => { if (e.key === "Enter") runSearch(); });
  ...
  function renderGalleryTab() {
    ...
    adminContent.innerHTML = `
      <div class="card">
        ...
        <div class="field-label" style="margin-bottom:.5rem">Upload a decorative frame with a <b>transparent centre (PNG only)</b>. Every photo you add to the gallery from now on is automatically placed inside this frame \u2014 the photo fills the transparent window, the frame's border/design stays fixed on top.</div>
      ...
      <div class="card">
        ...
        <div class="muted" style="font-size:.68rem;margin-top:.5rem">${frameUrl ? "Photo will be placed inside your uploaded frame automatically." : "Photos are auto-compressed (resized \u2264 1000px, ~70% quality) before saving \u2014 keeps them sharp but small so Firebase stays fast for everyone."}</div>
      </div>
    ...
    document.getElementById("btnAddPhoto").addEventListener("click", () => {
      ...
      compressImageFile(file, 1000, 0.72).then((dataUrl) => compositeWithFrame(dataUrl)).then((finalUrl) => {
  ...
  function openPrintSheet(kind, eventId) {
    ...
    const sectorLine = `${escapeHtml(state.hero.title)} THARUVANA Sector`;
    let body;
