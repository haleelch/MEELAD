...
(function () {
  ...
  function renderResultsList() {
    ...
    list.innerHTML = rows
      ? rows + `<button class="btn btn-primary" id="btnViewPoster" style="width:100%;margin-top:.5rem;padding:.75rem">\u{1F5BC} View &amp; Download Poster</button>`
      : `<div class="empty-note">Results not published yet for ${escapeHtml(event.name)}.</div>`;

    const posterBtn = document.getElementById("btnViewPoster");
    if (posterBtn) posterBtn.addEventListener("click", () => openPosterModal(event));
  }
  ...
  function drawTextBlock(ctx, cx, y, color, rankLabel, line1, line2, line3) {
    ...
    ctx.fillStyle = color; ctx.font = "bold 28px Georgia"; ctx.fillText(rankLabel, cx, y);
    ctx.font = "bold 50px Georgia"; ctx.fillText(line1, cx, y + 62);
    ctx.font = "26px Georgia"; ctx.fillText(line2, cx, y + 108);
    ctx.font = "22px Georgia"; ctx.fillText(line3, cx, y + 144);
    ctx.shadowBlur = 0;
  }

  // Returns a Promise<dataURL>. Image-based custom templates need to load
  // the background image first, so this whole thing is async.
  function drawWinnersBlock(ctx, cx, startY, color, winners) {
    ctx.textAlign = "center";
    ctx.shadowColor = "rgba(0,0,0,.65)"; ctx.shadowBlur = 8;
    winners.forEach((w, i) => {
      const y = startY + i * 128;
      ctx.fillStyle = color; ctx.font = "bold 24px Georgia";
      ctx.fillText(`${RANK_ICON[w.rank]}  ${RANK_LABEL[w.rank]}`, cx, y);
      ctx.font = "bold 34px Georgia";
      ctx.fillText(w.student.name, cx, y + 40);
      ctx.font = "19px Georgia";
      ctx.fillText(`${w.student.chestNo} \u00b7 ${w.team ? w.team.name : ""}`, cx, y + 68);
    });
    ctx.shadowBlur = 0;
  }

  // Draws all published winners (1st/2nd/3rd) for one programme into a single poster.
  function drawResultsPoster(event, winners, templateOverride) {
    const template = templateOverride || state.hero.posterTemplate || "classic";
    const custom = template.startsWith("custom-") ? state.customTemplates.find((ct) => ct.id === template) : null;
    ...
    if (custom && custom.imageUrl) {
      return loadImage(custom.imageUrl).then((img) => {
        ...
        const startY = canvas.height * ((custom.textY || 70) / 100);
        drawWinnersBlock(ctx, canvas.width / 2, startY, custom.textColor || "#FFFFFF", winners);
    ...
    if (custom) {
      ...
      grad.addColorStop(0, custom.bg1); grad.addColorStop(1, custom.bg2);
      ctx.fillStyle = grad; ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.strokeStyle = custom.fg; ctx.lineWidth = 6; ctx.strokeRect(30, 30, canvas.width - 60, canvas.height - 60);
      ctx.textAlign = "center";
      ctx.fillStyle = custom.fg; ctx.font = "bold 34px Georgia"; ctx.fillText(state.hero.title, canvas.width / 2, 150);
      ctx.font = "22px Georgia"; ctx.fillText(event.name, canvas.width / 2, 195);
      drawWinnersBlock(ctx, canvas.width / 2, 420, "#fff", winners);
      ...
      ctx.textAlign = "center";
      ctx.fillStyle = "#0B3D2E"; ctx.font = "600 26px Georgia"; ctx.fillText(state.hero.title.toUpperCase(), canvas.width / 2, 150);
      ctx.fillStyle = "#C9A227"; ctx.font = "600 20px Georgia"; ctx.fillText(event.name, canvas.width / 2, 190);
      drawWinnersBlock(ctx, canvas.width / 2, 420, "#0B3D2E", winners);
      ...
      ctx.textAlign = "center";
      ctx.fillStyle = "#E4C767"; ctx.font = "bold 34px Georgia"; ctx.fillText(state.hero.title, canvas.width / 2, 150);
      ctx.font = "italic 22px Georgia"; ctx.fillText(event.name, canvas.width / 2, 190);
      drawWinnersBlock(ctx, canvas.width / 2, 420, "#fff", winners);
    } else {
      ...
      ctx.textAlign = "center";
      ctx.fillStyle = "#FAF6EC"; ctx.font = "bold 40px Georgia"; ctx.fillText(state.hero.title, canvas.width / 2, 150);
      ctx.font = "22px Georgia"; ctx.fillStyle = "#C9A227"; ctx.fillText(event.name, canvas.width / 2, 195);
      drawWinnersBlock(ctx, canvas.width / 2, 430, "#FAF6EC", winners);
      ctx.fillStyle = "#9BBFAE"; ctx.font = "18px Georgia"; ctx.fillText(state.hero.title, canvas.width / 2, canvas.height - 60);
    }
  ...
  function drawPoster(payload, templateOverride) {
    ...
    if (custom && custom.imageUrl) {
      return loadImage(custom.imageUrl).then((img) => {
        ...
        const w = img.width * scale, h = img.height * scale;
        ctx.drawImage(img, (canvas.width - w) / 2, (canvas.height - h) / 2, w, h);
        const y = canvas.height * ((custom.textY || 75) / 100);
    ...
    if (custom) {
      ...
      ctx.strokeStyle = custom.fg; ctx.lineWidth = 6; ctx.strokeRect(30, 30, canvas.width - 60, canvas.height - 60);
      ctx.textAlign = "center";
      ctx.fillStyle = custom.fg; ctx.font = "bold 34px Georgia"; ctx.fillText(state.hero.title, canvas.width / 2, 220);
      ctx.font = "22px Georgia"; ctx.fillText(subtitle, canvas.width / 2, 265);
      drawTextBlock(ctx, canvas.width / 2, canvas.height * ((custom.textY || 55) / 100), "#fff", rankLabel, line1, line2, line3);
    } else if (template === "minimal") {
      ctx.fillStyle = "#FAF6EC"; ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.strokeStyle = "#0B3D2E"; ctx.lineWidth = 3; ctx.strokeRect(50, 50, canvas.width - 100, canvas.height - 100);
      ctx.textAlign = "center";
  ...
  function openPosterModal(event) {
    ...
    if (!winners.length) return showToast("No results to show yet for this programme");

    let selectedTemplate = state.hero.posterTemplate || "classic";
    const templates = getAllTemplates();

    modalBody.innerHTML = `
      <div class="poster-head arch-top">
        <div style="font-size:1.2rem;color:var(--gold)">\u2605</div>
        <div class="poster-event" style="margin-top:.2rem;font-weight:600">${escapeHtml(event.name)}</div>
        <div style="margin-top:.75rem;display:flex;flex-direction:column;gap:.55rem">
          ${winners.map((w) => `
            <div>
              <div class="poster-rank" style="font-size:.8rem">${RANK_ICON[w.rank]} ${RANK_LABEL[w.rank]}</div>
              <div class="poster-name font-display" style="font-size:1.2rem">${escapeHtml(w.student.name)}</div>
              <div class="poster-code">${w.student.chestNo} \u00b7 ${w.team ? escapeHtml(w.team.name) : ""}</div>
            </div>`).join("")}
        </div>
      </div>
      ${templates.length > 1 ? `
      <div style="padding:.85rem 1rem 0;background:var(--surface)">
        <div class="field-label" style="margin-bottom:.4rem">Swipe to choose a poster design</div>
        <div class="template-carousel" id="pickTemplateRow">
          ${templates.map((t) => `<div class="template-swatch template-swatch-lg ${t.id === selectedTemplate ? "selected" : ""}" data-tpl="${t.id}" style="background:${t.bg};color:${t.fg};text-shadow:0 1px 3px rgba(0,0,0,.6)">${t.label}</div>`).join("")}
        </div>
      </div>` : ""}
    ...
    document.querySelectorAll("#pickTemplateRow .template-swatch").forEach((sw) => {
      sw.addEventListener("click", () => {
        selectedTemplate = sw.dataset.tpl;
        document.querySelectorAll("#pickTemplateRow .template-swatch").forEach((s) => s.classList.remove("selected"));
    ...
    document.getElementById("btnDownloadPoster").addEventListener("click", () => {
      ...
      drawResultsPoster(event, winners, selectedTemplate).then((url) => {
        downloadDataUrl(url, `${event.name.replace(/\s+/g, "-")}-results.png`);
    ...
    document.getElementById("btnSharePoster").addEventListener("click", () => {
      const lines = winners.map((w) => `${RANK_LABEL[w.rank]}: ${w.student.name} (${w.team ? w.team.name : ""})`).join(", ");
      const text = encodeURIComponent(`Alhamdulillah! Results for ${event.name} at ${state.hero.title} \u2014 ${lines}`);
  ...
  document.querySelectorAll("#adminTabs [data-tab]").forEach((btn) => {
    btn.addEventListener("click", () => {
      document.querySelectorAll("#adminTabs [data-tab]").forEach((b) => b.classList.toggle("active", b === btn));
  ...
  function getAllTemplates() {
    return [
      ...
      ...state.customTemplates.map((t) => ({
        ...
        bg: t.imageUrl ? `url('${t.imageUrl}') center/cover` : `linear-gradient(135deg, ${t.bg1}, ${t.bg2})`,
        fg: t.imageUrl ? "#fff" : t.fg, custom: true,
      })),
    ];
  }
  function renderPosterTab() {
    ...
    adminContent.innerHTML = `
      ...
      <div class="card">
        ...
        <div class="field-label" style="margin-bottom:.5rem">Add up to 5 templates (a background image, or a colour gradient). Parents pick one when downloading a winner's poster \u2014 the name, chest number and programme are placed on top automatically.</div>
      ...
      <div class="card">
        ...
        <div class="field-label">Subtitle (shown in the top bar, next to the logo)</div>
        <input id="fSubtitle" class="input" style="margin-bottom:.5rem" value="${escapeAttr(h.subtitle)}" />
        <div class="field-label">Arabic Line (used on the "Classic" poster design)</div>
    ...
    document.getElementById("btnShowAddTemplate").addEventListener("click", () => {
      ...
      holder.innerHTML = `
        <div class="card" style="margin-bottom:.75rem">
          ...
          <div class="field-label">Background Image (optional \u2014 portrait works best, e.g. 1080\u00d71350px)</div>
          <input type="file" id="ntImage" accept="image/*" class="input" style="margin-bottom:.5rem;padding:.4rem" />
          <div class="field-label">...or pick colours instead (used only if no image is chosen)</div>
          ...
          <div class="field-label">Text Position (how far down the image the name/rank text sits)</div>
          <input id="ntTextY" type="range" min="30" max="90" value="75" style="width:100%;margin-bottom:.75rem" />
      ...
      document.getElementById("btnSaveTemplate").addEventListener("click", () => {
        ...
        const bg1 = document.getElementById("ntBg1").value;
        const bg2 = document.getElementById("ntBg2").value;
        const fg = document.getElementById("ntFg").value;
        const save = (imageUrl) => {
          state.customTemplates.push({ id: "custom-" + uid(), label: name, imageUrl, bg1, bg2, fg, textY, textColor: "#FFFFFF" });
          persist(); showToast("Template added"); renderPosterTab();
        };
        if (file) {
          showToast("Compressing template image\u2026");
          compressImageFile(file, 1200, 0.78).then(save).catch(() => showToast("Could not process that image"));
        } else {
          save(null);
        }
      });
    });
    document.querySelectorAll(".template-del").forEach((b) => b.addEventListener("click", (ev) => {
      ev.stopPropagation();
  ...
  function renderGalleryTab() {
    adminContent.innerHTML = `
      <div class="card">
        ...
        <div class="muted" style="font-size:.68rem;margin-top:.5rem">Photos are auto-compressed (resized \u2264 1000px, ~70% quality) before saving \u2014 keeps them sharp but small so Firebase stays fast for everyone.</div>
      </div>
    ...
    document.getElementById("btnAddPhoto").addEventListener("click", () => {
      ...
      compressImageFile(file, 1000, 0.72).then((dataUrl) => {
        state.gallery.push({ id: uid(), caption, color: "#155E43", url: dataUrl });
