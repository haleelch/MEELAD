...
(function () {
    ...
    list.innerHTML = pageEvents.map((event) => {
      ...
      <div class="rf-card ${expanded ? "expanded" : ""}" data-event="${event.id}">
        <div class="rf-header" data-toggle="${event.id}">
          ...
          ${!expanded ? `<div class="rf-preview-winner">${RANK_ICON.first} ${escapeHtml(first.student.name)}</div>` : ""}
          <span class="rf-chevron">${expanded ? "\u25B4" : "\u25BE"}</span>
        </div>
        ${expanded ? `
  ...
  // as a fixed 1080x1080 (Instagram square) design. The layout/position of
  // every text block is fixed by this function — a result template only ever
  // supplies the background image, nothing else needs configuring.
  function drawResultPoster(event, winners, templateOverride) {
    ...
    const paint = () => {
      ...
      ctx.textAlign = "center";
      ctx.fillStyle = "#fff";
      const smallLine = (state.hero.subtitle || state.hero.tag1 || "").toUpperCase();
      if (smallLine) { ctx.font = "600 22px Poppins, sans-serif"; ctx.fillText(smallLine, cx, 108); }
      ctx.font = "800 82px Georgia, serif";
      ctx.fillText(state.hero.title, cx, 205);
      ctx.font = "500 20px Poppins, sans-serif";
      const dateLine = new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" });
      ctx.fillText(dateLine, cx, 248);

      // category / event
      ctx.font = "600 32px Poppins, sans-serif";
      ctx.fillStyle = "#fff";
      ctx.fillText(event.category, cx, 348);
      ctx.font = "700 46px Poppins, sans-serif";
      ctx.fillStyle = "#EC4899";
      const eventLines = wrapText(ctx, event.name, canvas.width - 160);
      eventLines.forEach((ln, i) => ctx.fillText(ln, cx, 400 + i * 54));

      // results list
      let y = 480 + (eventLines.length - 1) * 54;
      winners.forEach((w, i) => {
        const cyc = y + 27;
        ctx.shadowBlur = 0;
        ctx.fillStyle = "rgba(0,0,0,.6)";
        ctx.beginPath(); ctx.arc(PAD_X + 27, cyc, 27, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#EC4899"; ctx.font = "700 28px Poppins, sans-serif"; ctx.textAlign = "center";
        ctx.fillText(String(i + 1), PAD_X + 27, cyc + 10);

        ctx.textAlign = "left";
        ctx.shadowColor = "rgba(0,0,0,.55)"; ctx.shadowBlur = 8;
        ctx.fillStyle = "#fff"; ctx.font = "700 36px Poppins, sans-serif";
        ctx.fillText(w.student.name.toUpperCase(), PAD_X + 78, y + 18);
        ctx.font = "400 23px Poppins, sans-serif"; ctx.fillStyle = "#D1D5DB";
        ctx.fillText(w.team ? w.team.name : "", PAD_X + 78, y + 48);
        ctx.shadowBlur = 0;
        ctx.textAlign = "center";
        y += 110;
      });
      ...
      ctx.fillText(`\u2661 \u{1F4AC} \u2197  Event Crew \u2022 ${state.hero.title}`, PAD_X, canvas.height - 50);
    };
    ...
    // Fallback flat dark background so the feature still works before any
    // result template has been uploaded.
