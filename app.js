...
(function () {
  ...
  function renderHero() {
    ...
    // Stats box background photo (optional, one photo for the whole box) —
    // text sits on top via a built-in dark overlay gradient.
    const statsCard = document.querySelector(".stats-card");
    if (statsCard) {
      if (state.hero.statsBoxPhotoUrl) {
        statsCard.style.backgroundImage = `linear-gradient(rgba(6,36,26,.5), rgba(6,36,26,.62)), url('${state.hero.statsBoxPhotoUrl}')`;
        statsCard.style.backgroundSize = "cover";
        statsCard.style.backgroundPosition = "center";
        statsCard.classList.add("has-bg-photo");
      } else {
        statsCard.style.backgroundImage = "";
        statsCard.classList.remove("has-bg-photo");
      }
    }
  }
  ...
  function renderLeaderboard() {
    ...
    document.getElementById("leaderboard").innerHTML = sorted.map((t, i) => {
      ...
      const dotClass = i === 0 ? "dot1" : i === 1 ? "dot2" : i === 2 ? "dot3" : "dot-other";
      return `
      <div class="team-row">
        <div class="rank-wrap">
          ...
          <div class="team-info">
            ...
            <span><span class="green-dot ${dotClass}" style="${i > 2 ? `background:${t.color}` : ""}"></span>${i === 0 ? "Leading" : "Current Position"}</span>
          </div>
        </div>
  ...
  const RESULTS_FEED_PAGE_SIZE = 5;
  ...
  function drawResultPoster(event, winners) {
    const canvas = document.createElement("canvas");
    canvas.width = 1080; canvas.height = 1080;
    const ctx = canvas.getContext("2d");
    const PAD_X = 50, PAD_TOP = 60;
    ...
    // pink brush glow, top-right
    const bx = canvas.width * 0.72, by = canvas.height * 0.18;
    const grad = ctx.createRadialGradient(bx, by, 0, bx, by, 420);
    grad.addColorStop(0, "rgba(236,72,153,.55)");
    grad.addColorStop(.5, "rgba(236,72,153,.22)");
    ...
    ctx.fillStyle = "#fff";
    const smallLine = (state.hero.subtitle || state.hero.badge || "").toUpperCase();
    if (smallLine) { ctx.font = "600 20px Poppins, sans-serif"; ctx.fillText(smallLine, cx, PAD_TOP + 22); }
    ctx.font = "800 80px Georgia, serif";
    ctx.fillText(state.hero.title, cx, PAD_TOP + 105);
    ctx.font = "500 18px Poppins, sans-serif";
    const dateLine = new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" });
    ctx.fillText(dateLine, cx, PAD_TOP + 145);

    // category + item (left-aligned)
    ctx.shadowBlur = 0;
    ctx.textAlign = "left";
    let y = PAD_TOP + 240;
    ctx.font = "600 24px Poppins, sans-serif"; ctx.fillStyle = "#fff";
    ctx.fillText(event.category, PAD_X, y);
    y += 50;
    ctx.font = "700 42px Poppins, sans-serif"; ctx.fillStyle = "#EC4899";
    const eventLines = wrapText(ctx, event.name, canvas.width - PAD_X * 2);
    eventLines.forEach((ln, i) => ctx.fillText(ln, PAD_X, y + i * 50));
    y += eventLines.length * 50 + 40;

    // result rows — rank circle left, name + team stacked to its right
    winners.forEach((w, i) => {
      const cyc = y + 25;
      ctx.textAlign = "center";
      ctx.fillStyle = "rgba(0,0,0,.6)";
      ctx.beginPath(); ctx.arc(PAD_X + 25, cyc, 25, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#EC4899"; ctx.font = "700 26px Poppins, sans-serif";
      ctx.fillText(String(i + 1), PAD_X + 25, cyc + 9);

      ctx.textAlign = "left";
      ctx.shadowColor = "rgba(0,0,0,.5)"; ctx.shadowBlur = 6;
      ctx.fillStyle = "#fff"; ctx.font = "700 36px Poppins, sans-serif";
      ctx.fillText(w.student.name.toUpperCase(), PAD_X + 68, y + 18);
      ctx.shadowBlur = 0;
      ctx.font = "400 20px Poppins, sans-serif"; ctx.fillStyle = "#D1D5DB";
      ctx.fillText(w.team ? w.team.name : "", PAD_X + 68, y + 48);
    ...
    ctx.font = "500 18px Poppins, sans-serif"; ctx.fillStyle = "#9CA3AF";
    const prefix = "\u2764\uFE0F \u{1F4AC} \u2197  Event Crew \u2022 ";
    ctx.fillText(prefix, PAD_X, footerY);
    const prefixWidth = ctx.measureText(prefix).width;
    ctx.font = "700 18px Poppins, sans-serif"; ctx.fillStyle = "#fff";
    ctx.fillText(state.hero.title, PAD_X + prefixWidth, footerY);
  ...
  function renderGallery() {
    ...
    if (pager) {
      if (totalPages <= 1) { pager.innerHTML = ""; }
        ...
        pager.innerHTML = `
          <button class="rf-page-btn" id="homeGalPrev" ${homeGalleryPage === 0 ? "disabled" : ""} aria-label="Previous page">&#171;</button>
          <span class="rf-page-info">${homeGalleryPage + 1} / ${totalPages}</span>
          <button class="rf-page-btn" id="homeGalNext" ${homeGalleryPage >= totalPages - 1 ? "disabled" : ""} aria-label="Next page">&#187;</button>`;
        const prevBtn = document.getElementById("homeGalPrev");
        const nextBtn = document.getElementById("homeGalNext");
        if (prevBtn) prevBtn.addEventListener("click", () => { homeGalleryPage--; renderGallery(); });
        if (nextBtn) nextBtn.addEventListener("click", () => { homeGalleryPage++; renderGallery(); });
      }
    }
  }
  ...
  /* ---- Dashboard tab ---- */
  /* ---------------- sample/test data (Magdad / Khurasan / Qurtuba batch) ----------------
     Loads a full demo dataset for testing (16 programmes, 3 teams, 60 students).
  ...
  const TEST_TEAMS = [
    { name: "Team Magdad", color: "#86EFAC" },   // light green
    { name: "Team Khurasan", color: "#FDE68A" }, // light yellow
    { name: "Team Qurtuba", color: "#93C5FD" },  // light blue
  ];
  const TEST_CATEGORY_EVENTS = {
    "Sub Junior": ["\u0d2e\u0d26\u0d4d\u0d39\u0d4d \u0d17\u0d3e\u0d28\u0d02", "\u0d2e\u0d3e\u0d2a\u0d4d\u0d2a\u0d3f\u0d33\u0d2a\u0d4d\u0d2a\u0d3e\u0d1f\u0d4d\u0d1f\u0d4d", "\u0d1a\u0d3f\u0d24\u0d4d\u0d30\u0d31\u0d1a\u0d28", "\u0d2e\u0d32\u0d2f\u0d3e\u0d33\u0d02 \u0d2a\u0d4d\u0d30\u0d38\u0d02\u0d17\u0d02"],
    "Junior": ["\u0d2e\u0d26\u0d4d\u0d39\u0d4d \u0d17\u0d3e\u0d28\u0d02", "\u0d2e\u0d3e\u0d2a\u0d4d\u0d2a\u0d3f\u0d33\u0d2a\u0d4d\u0d2a\u0d3e\u0d1f\u0d4d\u0d1f\u0d4d", "\u0d15\u0d25\u0d3e\u0d30\u0d1a\u0d28", "\u0d2e\u0d32\u0d2f\u0d3e\u0d33\u0d02 \u0d2a\u0d4d\u0d30\u0d38\u0d02\u0d17\u0d02"],
    "Senior": ["\u0d09\u0d31\u0d41\u0d26\u0d41 \u0d2a\u0d4d\u0d30\u0d38\u0d02\u0d17\u0d02", "\u0d2e\u0d32\u0d2f\u0d3e\u0d33\u0d02 \u0d2a\u0d4d\u0d30\u0d38\u0d02\u0d17\u0d02", "\u0d2e\u0d3e\u0d2a\u0d4d\u0d2a\u0d3f\u0d33\u0d2a\u0d4d\u0d2a\u0d3e\u0d1f\u0d4d\u0d1f\u0d4d", "\u0d15\u0d25\u0d3e\u0d30\u0d1a\u0d28"],
    "Super Senior": ["\u0d09\u0d31\u0d41\u0d26\u0d41 \u0d2a\u0d4d\u0d30\u0d38\u0d02\u0d17\u0d02", "\u0d2e\u0d32\u0d2f\u0d3e\u0d33\u0d02 \u0d2a\u0d4d\u0d30\u0d38\u0d02\u0d17\u0d02", "\u0d2e\u0d26\u0d4d\u0d39\u0d4d \u0d17\u0d3e\u0d28\u0d02", "\u0d15\u0d25\u0d3e\u0d30\u0d1a\u0d28"],
  };
  const TEST_STUDENTS = {
    "Team Magdad": {
      "Sub Junior": [["Ayan", [0, 2]], ["Rayan", [1, 3]], ["Zayan", [2, 1]], ["Izan", [0, 3]], ["Faizan", [2, 0]]],
      "Junior": [["Shamil", [0, 2, 3]], ["Nihal", [1, 2, 0]], ["Fadil", [3, 1, 2]], ["Sahal", [2, 0, 3]], ["Adil", [1, 3, 2]]],
      "Senior": [["Rashid", [0, 1, 3]], ["Fayiz", [2, 1, 0]], ["Zahid", [3, 0, 2]], ["Nabeel", [1, 3, 2]], ["Jasim", [0, 2, 1]]],
      "Super Senior": [["Irphan", [0, 1, 3]], ["Faris", [2, 0, 1]], ["Mansoor", [3, 2, 0]], ["Yaseen", [1, 3, 2]], ["Ameen", [0, 2, 1]]],
    },
    "Team Khurasan": {
      "Sub Junior": [["Ibrahim", [0, 2]], ["Yusuf", [1, 3]], ["Isa", [2, 1]], ["Yahya", [0, 3]], ["Musa", [2, 0]]],
      "Junior": [["Fahad", [0, 2, 3]], ["Farhan", [1, 2, 0]], ["Fuad", [3, 1, 2]], ["Firas", [2, 0, 3]], ["Fazil", [1, 3, 2]]],
      "Senior": [["Swalih", [0, 1, 3]], ["Sajid", [2, 1, 0]], ["Shafi", [3, 0, 2]], ["Saeed", [1, 3, 2]], ["Suhail", [0, 2, 1]]],
      "Super Senior": [["Hashim", [0, 1, 3]], ["Hamza", [2, 0, 1]], ["Hasan", [3, 2, 0]], ["Hussain", [1, 3, 2]], ["Haidar", [0, 2, 1]]],
    },
    "Team Qurtuba": {
      "Sub Junior": [["Razi", [0, 2]], ["Rami", [1, 3]], ["Rohan", [2, 1]], ["Ruwaid", [0, 3]], ["Riyash", [2, 0]]],
      "Junior": [["Tariq", [0, 2, 3]], ["Tahir", [1, 2, 0]], ["Tanveer", [3, 1, 2]], ["Tawfeeq", [2, 0, 3]], ["Taha", [1, 3, 2]]],
      "Senior": [["Kabir", [0, 1, 3]], ["Kamal", [2, 1, 0]], ["Karim", [3, 0, 2]], ["Kashif", [1, 3, 2]], ["Khalid", [0, 2, 1]]],
      "Super Senior": [["Ameir", [0, 1, 3]], ["Anwar", [2, 0, 1]], ["Akram", [3, 2, 0]], ["Ashraf", [1, 3, 2]], ["Azhar", [0, 2, 1]]],
    },
  };
  ...
  function loadTestData() {
    ...
    Object.keys(TEST_CATEGORY_EVENTS).forEach((cat) => {
      ...
      TEST_CATEGORY_EVENTS[cat].forEach((evName) => {
        const id = uid();
        state.events.push({ id, name: evName, category: cat, type: "Individual", gender: "General", status: "pending" });
    ...
    Object.keys(TEST_STUDENTS).forEach((teamName) => {
      ...
      Object.keys(byCategory).forEach((cat) => {
        const evNames = TEST_CATEGORY_EVENTS[cat];
        byCategory[cat].forEach(([name, evIdxList]) => {
          const chestNo = nextChestNo(cat);
          const id = uid();
          const events = evIdxList.map((i) => eventIdByCatName[cat][evNames[i]]);
          state.students.push({ id, name, cls: "", phone: "", gender: "Boys", team: teamId, category: cat, chestNo, events });
    ...
    showToast("Test data loaded \u2014 16 programmes, 3 teams, 60 students");
    renderDashboardTab();
  }
  ...
  function renderDashboardTab() {
    ...
    adminContent.innerHTML = `
      ...
      <div class="card" style="margin-top:1.25rem">
        ...
        <div class="field-label" style="margin-bottom:.6rem">Loads a demo dataset for trying things out \u2014 16 programmes, 3 teams (Magdad/Khurasan/Qurtuba), 60 students. Fully reversible: Delete removes exactly what Load added, nothing else.</div>
        ${isTestDataLoaded()
    ...
    if (btnDelTD) btnDelTD.addEventListener("click", () => {
      if (confirm("Delete all test data (3 teams, 60 students, 16 programmes)? This can't be undone.")) deleteTestData();
    });
  }
  ...
  function renderPosterTab() {
    ...
    adminContent.innerHTML = `
      ...
      <div class="card">
        <div class="card-title">Home Stats Box Background Photo</div>
        <div class="field-label" style="margin-bottom:.5rem">One background photo for the whole "at a glance" box (Total Programmes / Participants / Teams together) \u2014 not a separate photo per number. The numbers and labels stay on top.</div>
        ${h.statsBoxPhotoUrl ? `<div class="hero-photo-preview" style="background-image:url('${h.statsBoxPhotoUrl}');height:4.5rem;margin-bottom:.4rem"></div>` : ""}
        <input type="file" id="statsBoxPhotoFile" accept="image/*" class="input" style="margin:.5rem 0;padding:.4rem" />
        <div style="display:flex;gap:.5rem">
          <button class="btn btn-primary" id="btnSaveStatsBoxPhoto" style="width:auto;padding:.5rem .9rem">${h.statsBoxPhotoUrl ? "Replace Photo" : "Upload Photo"}</button>
          ${h.statsBoxPhotoUrl ? `<button class="btn btn-ghost" id="btnRemoveStatsBoxPhoto" style="width:auto;padding:.5rem .9rem">Remove</button>` : ""}
        </div>
      </div>
    ...
    document.getElementById("btnSaveStatsBoxPhoto").addEventListener("click", () => {
      const file = document.getElementById("statsBoxPhotoFile").files[0];
      if (!file) return showToast("Choose a photo first");
      showToast("Uploading\u2026");
      compressImageFile(file, 900, 0.7).then((dataUrl) => {
        state.hero.statsBoxPhotoUrl = dataUrl;
        persist(); renderHero(); renderPosterTab();
        showToast("Stats box background photo updated");
      }).catch(() => showToast("Could not process that photo"));
    });
    const removeStatsBoxPhotoBtn = document.getElementById("btnRemoveStatsBoxPhoto");
    if (removeStatsBoxPhotoBtn) removeStatsBoxPhotoBtn.addEventListener("click", () => {
      delete state.hero.statsBoxPhotoUrl;
      persist(); renderHero(); renderPosterTab();
      showToast("Stats box background photo removed");
    });
    ...
    document.getElementById("btnShowAddResultTemplate").addEventListener("click", () => {
      ...
      holder.innerHTML = `
        <div class="card" style="margin-bottom:.75rem">
          <div class="field-label">Result Frame Image \u2014 square, 1080\u00d71080px recommended</div>
  ...
  function renderCardsTab() {
    ...
    adminContent.innerHTML = `
      ...
      <div class="card">
        ...
        <button class="btn btn-primary" id="btnPrintCategoryCards" style="width:auto;padding:.5rem .9rem">\u{1F5A8} Chest No PRINT</button>
      </div>`;
