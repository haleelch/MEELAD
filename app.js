...
(function () {
  ...
  function renderHero() {
    ...
    // Per-stat-box background photos (optional) — text sits on top via a
    // built-in dark overlay gradient. Falls back to the plain card look
    // when no photo has been set for that box.
    const statPhotos = state.hero.statPhotos || {};
    ["Programmes", "Participants", "Teams", "Points"].forEach((key) => {
      const el = document.getElementById("statBox" + key);
      if (!el) return;
      const url = statPhotos[key.toLowerCase()];
      if (url) {
        el.style.backgroundImage = `linear-gradient(rgba(6,36,26,.52), rgba(6,36,26,.65)), url('${url}')`;
        el.classList.add("has-bg-photo");
      } else {
        el.style.backgroundImage = "";
        el.classList.remove("has-bg-photo");
      }
    });
  }
  ...
  function renderPosterTab() {
    ...
    adminContent.innerHTML = `
      ...
      <div class="card">
        <div class="card-title">Home Boxes Background Photos</div>
        <div class="field-label" style="margin-bottom:.5rem">Optional background photo for each of the 4 home page boxes (Total Programmes, Registered Participants, Competing Teams, Total Points). The number and label stay on top of the photo.</div>
        ${["programmes", "participants", "teams", "points"].map((key) => {
          const label = { programmes: "Total Programmes", participants: "Registered Participants", teams: "Competing Teams", points: "Total Points" }[key];
          const url = (h.statPhotos || {})[key];
          return `
          <div style="margin-bottom:.85rem;padding-bottom:.85rem;border-bottom:1px solid var(--border)">
            <div class="field-label" style="margin-bottom:.35rem">${label}</div>
            ${url ? `<div class="hero-photo-preview" style="background-image:url('${url}');height:3.5rem;margin-bottom:.4rem"></div>` : ""}
            <div style="display:flex;gap:.5rem;align-items:center">
              <input type="file" data-statkey="${key}" accept="image/*" class="input stat-photo-file" style="padding:.4rem;flex:1" />
              <button class="btn btn-primary stat-photo-save" data-statkey="${key}" style="width:auto;padding:.5rem .8rem">${url ? "Replace" : "Upload"}</button>
              ${url ? `<button class="btn btn-ghost stat-photo-remove" data-statkey="${key}" style="width:auto;padding:.5rem .8rem">Remove</button>` : ""}
            </div>
          </div>`;
        }).join("")}
      </div>
    ...
    document.querySelectorAll(".stat-photo-save").forEach((btn) => {
      btn.addEventListener("click", () => {
        const key = btn.dataset.statkey;
        const fileInput = document.querySelector(`.stat-photo-file[data-statkey="${key}"]`);
        const file = fileInput.files[0];
        if (!file) return showToast("Choose a photo first");
        showToast("Uploading\u2026");
        compressImageFile(file, 700, 0.7).then((dataUrl) => {
          state.hero.statPhotos = state.hero.statPhotos || {};
          state.hero.statPhotos[key] = dataUrl;
          persist(); renderHero(); renderPosterTab();
          showToast("Box background photo updated");
        }).catch(() => showToast("Could not process that photo"));
      });
    });
    document.querySelectorAll(".stat-photo-remove").forEach((btn) => {
      btn.addEventListener("click", () => {
        const key = btn.dataset.statkey;
        if (state.hero.statPhotos) delete state.hero.statPhotos[key];
        persist(); renderHero(); renderPosterTab();
        showToast("Box background photo removed");
      });
    });
