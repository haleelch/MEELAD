...
(function () {
  ...
  function defaultCardPlacements() {
    return [
      { id: "pl-" + uid(), tag: "student_name", fontSize: 78, weight: 700, color: "#000" },
      { id: "pl-" + uid(), tag: "chest_number", fontSize: 72, weight: 600, color: "#000" },
      { id: "pl-" + uid(), tag: "category", fontSize: 72, weight: 600, color: "#000" },
      { id: "pl-" + uid(), tag: "team_name", fontSize: 55, weight: 400, color: "#000" },
    ];
  }
  ...
  function drawMasterCard(student) {
    ...
    return templatePromise.then((img) => {
      ...
      // Build each rendered "line" (name may wrap to 2 lines) up front so we
      // can measure them and center the whole right-anchored text block.
      // gapBefore is the GAP margin between separate fields, or 0 between
      // wrapped lines of the same name (they use tight line-height instead).
      const lines = [];
      placements.forEach((p) => {
        const val = resolveCardTag(p.tag, student);
        if (!val) return;
        const font = `${p.weight || 600} ${p.fontSize || 60}px Rajdhani, Poppins, sans-serif`;
        const uppercase = p.tag !== "chest_number";
        if (p.tag === "student_name") {
          val.split(" ").forEach((word, i) => lines.push({ text: uppercase ? word.toUpperCase() : word, font, fontSize: p.fontSize || 60, color: p.color || "#000", lineHeight: (p.fontSize || 60) * 1.2, gapBefore: i === 0 ? GAP : 0 }));
        } else {
          lines.push({ text: uppercase ? val.toUpperCase() : val, font, fontSize: p.fontSize || 60, color: p.color || "#000", lineHeight: (p.fontSize || 60) * 1.2, gapBefore: GAP });
        }
      });
      if (lines.length) lines[0].gapBefore = 0;

      const totalHeight = lines.reduce((sum, l) => sum + l.lineHeight + l.gapBefore, 0);
      let maxWidth = 0;
  ...
  function renderChestNumberTab() {
    ...
    adminContent.innerHTML = `
      ...
      <div class="card">
        ...
        <div class="field-label" style="margin-bottom:.5rem">Upload <b>one</b> template design (portrait, recommended 1080\u00d72337px). Name, Chest No, Category and Team are placed on it automatically.</div>
    ...
    if (mtInput) mtInput.addEventListener("change", () => {
      ...
      if (!file) return;
      showToast("Compressing template image\u2026");
      compressImageFile(file, 1080, 0.85).then((imageUrl) => {
