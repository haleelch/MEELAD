...
(function () {
  ...
  // Total marks a student has accumulated across every programme they're registered
  ...
  // of an admin-uploaded 1080x1350 background (flowers/decorative art —
  // baked into the uploaded JPG itself, not drawn here). Every text field
  // (category, programme name, rank ordinal, winner name, team) is filled
  // in automatically from the event/winners data at generation time.
  function drawWavyLines(ctx, x, y, w, h, rows, color, alpha, lineWidth) {
    ctx.save();
    ...
    for (let r = 0; r < rows; r++) {
      ...
      const step = w / 2;
      for (let s = 0; s < 2; s++) {
        ctx.quadraticCurveTo(x + step * s + step / 2, cy - rowH * 0.35, x + step * (s + 1), cy);
      }
      ctx.stroke();
    }
    ctx.restore();
  ...
  function drawFlowerCluster(ctx, x, y, scale) {
    ctx.save();
    ctx.translate(x, y);
    const bloom = (cx, cy, r, petalColor) => {
      ctx.beginPath(); ctx.arc(cx * scale, cy * scale, r * scale, 0, Math.PI * 2);
      ctx.fillStyle = petalColor; ctx.fill();
      ctx.beginPath(); ctx.arc(cx * scale, cy * scale, r * scale * 0.31, 0, Math.PI * 2);
      ctx.fillStyle = "#121212"; ctx.fill();
    };
    const leaf = (cx, cy, rx, ry, rot) => {
      ctx.save(); ctx.translate(cx * scale, cy * scale); ctx.rotate((rot * Math.PI) / 180);
      ctx.beginPath(); ctx.ellipse(0, 0, rx * scale, ry * scale, 0, 0, Math.PI * 2);
      ctx.fillStyle = "#4CAF50"; ctx.fill(); ctx.restore();
    };
    const butterfly = (cx, cy) => {
      ctx.save(); ctx.translate(cx * scale, cy * scale);
      ctx.fillStyle = "#fff";
      ctx.beginPath(); ctx.ellipse(-4 * scale, 0, 4 * scale, 3 * scale, 0.5, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.ellipse(4 * scale, 0, 4 * scale, 3 * scale, -0.5, 0, Math.PI * 2); ctx.fill();
      ctx.restore();
    };
    leaf(95, 115, 10, 18, -25); leaf(150, 120, 10, 18, 25); leaf(225, 145, 9, 16, -15);
    bloom(90, 80, 45, "#F4A22A"); bloom(160, 90, 40, "#FFC53D"); bloom(220, 115, 38, "#FF7A45");
    bloom(60, 135, 22, "#FFC53D"); bloom(110, 145, 24, "#F4A22A");
    bloom(270, 130, 20, "#FFC53D"); bloom(320, 145, 18, "#F4A22A");
    butterfly(100, 68); butterfly(292, 118);
    ctx.restore();
  }

  // Result poster: category + programme name + top-3 winners. Everything is
  // drawn by code (dark background, corner wave patterns, a soft pink/purple
  // glow, the rank badges, and the flower cluster at the bottom) \u2014 no
  // admin-uploaded background image is needed for this design any more.
  function drawResultPoster(event, winners) {
    const canvas = document.createElement("canvas");
    canvas.width = 1080; canvas.height = 2337;
    const ctx = canvas.getContext("2d");
    const CW = canvas.width, CH = canvas.height;

    ctx.fillStyle = "#121212"; ctx.fillRect(0, 0, CW, CH);

    drawWavyLines(ctx, CW - 560, -110, 700, 660, 8, "#C9A961", 0.15, 5);
    drawWavyLines(ctx, -250, 70, 700, 560, 6, "#666", 0.08, 4);

    const glow = ctx.createRadialGradient(150, CH - 350, 0, 150, CH - 350, 700);
    glow.addColorStop(0, "rgba(233,30,99,0.5)");
    glow.addColorStop(0.5, "rgba(156,39,176,0.25)");
    glow.addColorStop(1, "rgba(156,39,176,0)");
    ctx.fillStyle = glow;
    ctx.beginPath(); ctx.arc(150, CH - 350, 700, 0, Math.PI * 2); ctx.fill();

    const PAD_L = 78;
    ctx.textAlign = "left";
    ctx.fillStyle = "#E8A44A"; ctx.font = "600 78px Poppins, sans-serif";
    ctx.fillText(event.category || "", PAD_L, 177 + 78 * 0.85);

    ctx.fillStyle = "#fff"; ctx.font = "700 111px Poppins, sans-serif";
    ctx.fillText(event.name || "", PAD_L, 256 + 111 * 0.85);

    const BOX = 177, RADIUS = 39, DETAILS_LEFT = PAD_L + BOX + 55;
    const ROW_TOP = [511, 766, 1021];
    ...
    winners.slice(0, 3).forEach((w, i) => {
      ...
      ctx.fillText(`${i + 1}${ordinalSuffix(i + 1)}`, PAD_L + BOX / 2, centerY);
      ctx.textBaseline = "alphabetic"; ctx.textAlign = "left";

      ctx.fillStyle = "#fff"; ctx.font = "700 66px Poppins, sans-serif";
      ctx.fillText(w.student.name || "", DETAILS_LEFT, centerY - 5);

      ctx.fillStyle = "#fff"; ctx.font = "italic 400 50px Poppins, sans-serif";
      ctx.fillText(w.team ? w.team.name : "", DETAILS_LEFT, centerY + 61);
    });

    drawFlowerCluster(ctx, 0, CH - 609, 2.769230769);

    ctx.textAlign = "center"; ctx.globalAlpha = 0.85;
    ctx.fillStyle = "#fff"; ctx.font = "400 33px Poppins, sans-serif";
    ctx.fillText("E V E N T    C R E W", CW / 2, CH - 60);
  ...
  /* ---- Chest Number Card (single master template) ----
     One admin-designed landscape template (1000x750px) with placeholder
  ...
  // the approved reference design: a solid name-banner box (drawn here, not
  // baked into the template JPG) with the student's name wrapped one word
  // per line inside it, then chest no / category / team stacked below.
  function defaultCardPlacements() {
    return [
      { id: "pl-" + uid(), tag: "name_box", xPct: 68, yPct: 17, wPct: 34, hPct: 14, color: "#c91d20" },
      { id: "pl-" + uid(), tag: "student_name", xPct: 68, yPct: 17, fontSize: 34, weight: 700, color: "#fff", align: "center" },
      { id: "pl-" + uid(), tag: "chest_number", xPct: 67.5, yPct: 30.5, fontSize: 40, weight: 500, color: "#fff", align: "center" },
      { id: "pl-" + uid(), tag: "category", xPct: 67.5, yPct: 43.2, fontSize: 40, weight: 500, color: "#fff", align: "center" },
      { id: "pl-" + uid(), tag: "team_name", xPct: 67.5, yPct: 54.7, fontSize: 34, weight: 300, color: "#e8e8e8", align: "center" },
  ...
  function drawMasterCard(student) {
    ...
    return Promise.all([templatePromise, qrPromise]).then(([img, qrImg]) => {
      if (img) {
        ...
        ctx.fillStyle = "#9CA3AF"; ctx.textAlign = "center"; ctx.font = "24px Poppins, sans-serif";
      ...
      (mt.placements || []).forEach((p) => {
        ...
        } else if (p.tag === "name_box") {
          const w = (p.wPct / 100) * canvas.width, h = (p.hPct / 100) * canvas.height;
          ctx.fillStyle = p.color || "#c91d20";
          ctx.fillRect(x - w / 2, y - h / 2, w, h);
        } else {
          const val = resolveCardTag(p.tag, student);
          if (!val) return;
          const weight = p.weight || (p.bold ? 700 : 500);
          ctx.fillStyle = p.color || "#111827";
          ctx.font = `${weight} ${p.fontSize || 30}px Poppins, sans-serif`;
          ctx.textAlign = p.align || "center";
          ctx.textBaseline = "middle";
          if (p.tag === "student_name") {
            // Wrap one word per line, matching the reference design.
            const lines = val.split(" ");
            const lineHeight = (p.fontSize || 30) * 1.25;
  ...
  function openResultPosterModal(eventId) {
    ...
    drawResultPoster(event, winners).then((url) => {
    ...
    document.getElementById("btnShareResult").addEventListener("click", () => {
      ...
      drawResultPoster(event, winners)
  ...
  function renderChestNumberTab() {
    ...
    adminContent.innerHTML = `
      ...
      <div class="card">
        ...
        <div class="field-label" style="margin-bottom:.5rem">Upload <b>one</b> template design (landscape, recommended 1000\u00d7750px). Name, Chest No, Category, Team and QR Code are placed on it automatically.</div>
    ...
    if (mtInput) mtInput.addEventListener("change", () => {
      ...
      if (!file) return;
      showToast("Compressing template image\u2026");
      compressImageFile(file, 1000, 0.85).then((imageUrl) => {
