...
(function () {
  ...
  function drawResultPoster(event, winners, templateOverride) {
    ...
    const ROW_TOP = [460, 650, 840];
    const ROW_LEFT = 210, BOX_SIZE = 120, BOX_RADIUS = 24, GAP = 70;
    const DETAILS_LEFT = ROW_LEFT + BOX_SIZE + GAP;

    const paintWinners = () => {
      ctx.shadowBlur = 0;

      // Category + programme title
      ctx.textAlign = "left";
      ctx.fillStyle = "#f2a93b"; ctx.font = "600 54px Poppins, sans-serif";
      ctx.fillText(event.category || "", ROW_LEFT, 235 + 54 * 0.85);

      ctx.fillStyle = "#fff"; ctx.font = "600 82px Poppins, sans-serif";
      ctx.fillText(event.name || "", ROW_LEFT, 308 + 82 * 0.85);

      winners.slice(0, 3).forEach((w, i) => {
        const top = ROW_TOP[i];
        const centerY = top + BOX_SIZE / 2;

        // Rank badge (rounded square)
        ctx.fillStyle = "#F3AE3B";
        roundedRectPath(ctx, ROW_LEFT, top, BOX_SIZE, BOX_SIZE, BOX_RADIUS);
        ctx.fill();

        ctx.fillStyle = "#202020";
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.font = "700 72px Poppins, sans-serif";
        ctx.fillText(String(i + 1), ROW_LEFT + BOX_SIZE / 2 - 14, centerY);
        ctx.textAlign = "left";
        ctx.font = "600 30px Poppins, sans-serif";
        ctx.fillText(ordinalSuffix(i + 1), ROW_LEFT + BOX_SIZE - 44, top + 54);
        ctx.textBaseline = "alphabetic";

        // Name + team
        ctx.textAlign = "left";
        ctx.fillStyle = "#fff"; ctx.font = "italic 600 64px Poppins, sans-serif";
        ctx.fillText(w.student.name || "", DETAILS_LEFT, centerY - 6);

        ctx.fillStyle = "#ececec"; ctx.font = "italic 400 42px Poppins, sans-serif";
        ctx.fillText(w.team ? w.team.name : "", DETAILS_LEFT, centerY + 46);
      });
    };
